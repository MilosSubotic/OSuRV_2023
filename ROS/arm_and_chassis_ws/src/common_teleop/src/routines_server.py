...
class RoutineExecutor:
	def __init__(self, routine):
		self.routine_basename = os.path.basename(routine)
		self.routine_dir = os.path.join(
			routines_dir,
			os.path.dirname(routine)
		)
		#TODO Warn on non-existing file.
		self.ext = get_ext(routine)
	...
	def include(self, routine_basename):
		fn = os.path.join(self.routine_dir, routine_basename)
		with open(fn) as f:
...
class RoutinesServer:
	...
	def go_routine_mode_and_exec(self, routine_relpath):
		rospy.loginfo('Running routine: {}'.format(routine_relpath))
		...
		RoutineExecutor(routine_relpath).run()

		# Return back.
		#TODO Make a diff which arm is in question.
		#TODO Need to read which controller was active.
		change_mode('servo') # For s3a
		#change_mode('jog') # Easier while debugging on b4a.


	def on_run_routine(self, msg):
		routine = msg.data
		self.go_routine_mode_and_exec(routine)

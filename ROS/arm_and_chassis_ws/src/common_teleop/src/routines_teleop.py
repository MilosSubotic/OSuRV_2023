...
class RoutinesTeleop:
	def __init__(self):
		...
		self.routines_path = rospy.get_param('~routines_path').split(':')
...
Routines Teleop:
	...
	def get_list_of_routines(self):
		r = []
		for path in self.routines_path:
			for ext in ['*.moveitcmd', '*.chassiscmd']:
				p = os.path.join(path, ext)
				g = glob.glob(p)
				for gg in g:
					#TODO Active routines.
					short_path = os.path.relpath(gg, self.routines_dir)
					b = os.path.basename(short_path)

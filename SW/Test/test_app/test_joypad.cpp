...
private:
  ...
  {
    double now = ros::Time::now().toSec(); //Dobavlja trenutno vreme u sekundama koristeći ROS Time klasu
    double interval = now - lastDiagTime_; //Racuna vremenski interval od poslednjeg dijagnostickog pracenja
    if (open_)
    {
      stat.summary(0, "OK");
    }
    ...
    event_count_ = 0; // brojaci koji prate broj dogadjaja i broj objava u odredjenom vremenskom intervalu
    pub_count_ = 0;
    lastDiagTime_ = now; //azurira na trenutno vreme
  }

  /*! \brief Returns the device path of the first joystick that matches joy_name.
   *         If no match is found, an empty string is returned.
   */
  //koristi se za pronalaženje uređaja (joysticka) na osnovu zadatog imena joysticka
  std::string get_dev_by_joy_name(const std::string& joy_name)
  {
    const char path[] = "/dev/input"; //Postavlja putanju do direktorijuma gde se nalaze ulazni uredjaji
    //Definisu strukture koje se koriste za citanje informacija o direktorijumu i uredjajima
    struct dirent *entry; 
    struct stat stat_buf;

    //pokazivac (dir_ptr) na otvoreni direktorijum /dev/input koristeci funkciju opendir i pridruzuje mu callback funkciju closedir_cb za zatvaranje direktorijuma
   ...
   */
   //koristi se za pronalazenje uredjaja za povratnu informaciju (force feedback) koji odgovara odabranom uredjaju za upravljanje (joysticku)
  std::string get_ff_dev(const std::string& joy_dev)
  {
    const char path[] = "/dev/input/by-id";  //Postavlja putanju do direktorijuma koji sadrži simbolicke linkove ka uredjajima prema njihovim jedinstvenim identifikatorima
    struct dirent *entry; //
